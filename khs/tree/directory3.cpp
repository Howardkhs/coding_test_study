...
vector<vector<int*>> node_v;

void init(int n){
    fill(node_num, node_num + n, 0); 
    fill(node_sum, node_sum + n, 0);
    node_sum[0] = 1;
    node_cnt = 1;
    node_v.clear();
    for (int i = 0; i < n; i++){
        vector<int *> map1; // 이걸 벡터로 바꿔보자
...
int find_v(vector<P*> &v, string name){
    int start = 0, end = distance(v.begin(), v.end()) - 1;
    while (start <= end){
        int mid = (start + end) / 2;
        if (v[mid]->first == name)
            return mid;
...
long long name_to_int(char *name){
    long long my_name = 0;
...
void cmd_mkdir(char path[PATH_MAXLEN + 1], char name[NAME_MAXLEN + 1]){
	...
	while(result != NULL){
		...
		if (parsing[0] =='\0')
			break;
        long long iname = name_to_int(parsing);
        node = node_v[node][find_v(node_v[node], str_buf)]->second;
		result = strtok(NULL,"/");
	}

    string _name(name);
    P* _ndir = new P(name, node_cnt);
    node_num[node_cnt] = node;  // parent 생성 1->0
    insert_v(node_v[node], _ndir); // childs 생성
...
void cmd_rm(char path[PATH_MAXLEN + 1]){
    ...
    int idx = find_v(node_v[node_num[node]], _name);
    node_v[node_num[node]].erase(node_v[node_num[node]].begin() + idx);
...
int copy(int node){
    ...
    for (auto p = node_v[node].begin(); p != node_v[node].end(); p++){
        ...
        P* n_p = new P((*p)->first, child_node);
        node_v[now_node].push_back(n_p);
    }
    return now_node;
...
void cmd_cp(char srcPath[PATH_MAXLEN + 1], char dstPath[PATH_MAXLEN + 1]){
	...
	while(result != NULL){
		...
		if (parsing[0] =='\0')
			break;
		string str_buf(parsing);
        _name = parsing; // 이름 복사해서 나중에 붙일 떄 사용
        node = node_v[node][find_v(node_v[node], str_buf)]->second;
		result = strtok(NULL,"/");
	}
    ...
    P* _ndir = new P(_name, c_node); // 이건 확인을 나중에
    insert_v(node_v[node], _ndir); // childs 생성
    // cout << node << "의 자식들";
    // for (auto p = node_v[node].begin(); p != node_v[node].end(); p++){
    //     cout << (*p)->first << " ";
    ...
    // cout << endl;
    // cout << c_node << "의 자식들";
    // for (auto p = node_v[c_node].begin(); p != node_v[c_node].end(); p++){
    //     cout << (*p)->first << " ";
    // }
    // cout << endl;
